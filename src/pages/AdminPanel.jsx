...
const AdminPanel = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const userRes = await API.get("/api/admin/users", authHeader);
      const taskRes = await API.get("/api/tasks/all", authHeader);
  ...
  const updateTaskStatus = async (id, status) => {
    try {
      await API.put(`/api/tasks/${id}`, { status }, authHeader);
      fetchData();
    } catch (err) {
      console.log(err);
      alert("Task update failed ❌");
